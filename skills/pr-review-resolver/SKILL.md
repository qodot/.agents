...
- 현재 코드: {코멘트가 가리키는 코드를 그대로 인용}
- 무엇이 문제인가: {리뷰어가 지적한 문제를 코드 컨텍스트 기준으로 풀어서 설명}
- 왜 문제인가: {그대로 두면 생기는 영향. 영향이 불확실하면 불확실하다고 명시}
...
- 문제 분석은 코멘트 요약이 아니라, 실제 코드를 읽고 확인한 내용으로 작성한다
- 모든 해결책 후보에 diff와 장단점을 붙인다. diff 없이 말로만 설명하지 않는다
- 리뷰어의 제안이 명확하면 그것을 A안으로 포함한다
- 코멘트 내용과 실제 코드 상태가 다르면(이미 반영됨, 잘못된 지적 등) 그 사실을 문제 분석에 명시한다
